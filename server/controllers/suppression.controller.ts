...
export class SuppressionController {
  public static async list(req: Request, res: Response): Promise<void> {
    ...
    const data = await suppressionRepository.list(limit);
    res.json({ success: true, data });
  }

  public static async add(req: AuthenticatedRequest, res: Response): Promise<void> {
  ...
  public static async remove(req: AuthenticatedRequest, res: Response): Promise<void> {
    const { email } = req.params;
    const ok = await suppressionRepository.remove(email);
